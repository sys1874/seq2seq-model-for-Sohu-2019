...
class RNNEncoder_pos(nn.Module):
    """
    ...
    def __init__(self,
                 ...
                 bidirectional=True,
                 dropout=0.0):
        super(RNNEncoder_pos, self).__init__()
    ...
    def forward(self, inputs, pos_inputs, hidden=None):
        """
        forward
        """
        ...
        pos_inputs = self.pos_embedder(pos_inputs)

        rnn_inputs = torch.cat([rnn_inputs, pos_inputs], dim=-1)
        ...
        if lengths is not None:
            ...
            if num_valid < batch_size:
                ...
                last_hidden[0] = torch.cat([last_hidden[0], zeros], dim=1)

                zeros = last_hidden[0].new_zeros(
                    self.num_layers, batch_size - num_valid, self.hidden_size)
                last_hidden[1] = torch.cat([last_hidden[1], zeros], dim=1)


            _, inv_indices = indices.sort()
            outputs = outputs.index_select(0, inv_indices)
            last_hidden[0] = last_hidden[0].index_select(1, inv_indices)
            last_hidden[1] = last_hidden[1].index_select(1, inv_indices)
    ...
    def _bridge_bidirectional_hidden(self, hidden):
        ...
        temp=[]
        for item in hidden:
            num_layers = item.size(0) // 2
            _, batch_size, hidden_size = item.size()
            transfer=item.view(num_layers, 2, batch_size, hidden_size) \
