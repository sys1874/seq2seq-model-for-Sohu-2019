...
class Pack(dict):
    ...
    def cuda(self, device=None):
        ...
        for k, v in self.items():
            if k =='raw_src' or k=='raw_tgt' or k=='id' or k=='vocab':
                pack[k]=v
            else:
...
def map2tensor(X):
    src_size = max([len(t) for t in X])
    src_vocab_size = max([max(t) for t in X]) + 1
    # alignment = torch.zeros(src_size, len(X), src_vocab_size)
    alignment = torch.zeros(len(X), src_size, src_vocab_size)
...
def align2tensor(X):
    tgt_size = max([len(t) for t in X])
    # alignment = torch.zeros(tgt_size, len(X)).long()
    alignment = torch.zeros(len(X), tgt_size).long()
    for i, sent in enumerate(X):
        # alignment[:len(sent), i] = torch.LongTensor(sent)
        alignment[i, :len(sent)] = torch.LongTensor(sent)
    return alignment
